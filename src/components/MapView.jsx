...
export default function MapView({
  ...
  const legendRoutes = [...safeRoutesInfo]
    .filter(Boolean)
    .sort((a, b) => {
      const aIdx = a?.originalIndex ?? a?.id ?? 0;
      const bIdx = b?.originalIndex ?? b?.id ?? 0;

      const aDisplay = displayOrder?.[aIdx] ?? aIdx;
      const bDisplay = displayOrder?.[bIdx] ?? bIdx;
  ...
  const setPointWithGeocode = (lat, lng, setPoint, setQuery, kind) => {
    ...
    if (kind === "start") {
      ...
      setAnnouncement("End point selected. You can now compare route options.");
    }
  ...
  useEffect(() => {
    ...
    if (focusRequest.target === "compute") {
      window.requestAnimationFrame(() => {
        focusWithoutScroll(computeButtonRef.current);
      });
    }
  }, [focusRequest]);
  ...
  const buildLegendCardStyle = (isOpen) => ({
  ...buildInfoCardStyle(isOpen),
  ...
  return (
    ...
    >
      ...
      <p id="route-map-description" style={srOnlyStyle}>
        Interactive map focused on the Camden study area. You can click the map
        to place start and end points, or use the search boxes in the sidebar.
        Areas outside Camden are visually de-emphasised to clarify data
        coverage. Route details and comparisons are also available as text
        outside the map.
      </p>
        ...
        {routeCount > 0 && (
          <div style={mapLegendStackStyle}>
            <section
              ...
              style={buildLegendCardStyle(showMapLegend)}
            >
  ...
  >
                  <div style={{ marginBottom: 14 }}>
                    ...
                    >
                      ...
                      >
                        ...
                        />
                        <span>
                          Higher air-pollution sample: NO₂ &gt; 40 μg/m³ or
                          PM2.5 &gt; 15 μg/m³
                        </span>
                      </li>
                        ...
                        return (
                          <li
                            ...
                            style={{
                              ...
                              alignItems: "center",
                              gap: 8,
                              marginBottom: index === legendRoutes.length - 1 ? 0 : 8,
                              fontSize: 12,
                              color: "#334155",
                              lineHeight: 1.45,
                            }}
        ...
        {shouldRenderStudyAreaCard && (
          ...
          >
            ...
            {showStudyAreaCard && (
              ...
              >
                Areas outside Camden are faded to show the study-area focus. This
                is a data boundary, not a safety boundary.
              </div>
            )}
          </section>
        )}
      ...
      <p id="map-controls-help" style={srOnlyStyle}>
        Use these controls to calculate routes, reset the map, centre the study
        area, zoom in or out, and toggle the Camden study-area focus.
      </p>
      ...
      >
        <button
          ...
          style={{
            ...
            color: canCompute ? "#FFFFFF" : "#64748B",
        ...
        <button
          ...
          aria-label={
            focusStudyArea
              ? "Show full map context"
              : "Focus on Camden study area"
          }
          className="map-control-button"
          style={{
            minHeight: 44,
            minWidth: 44,
            padding: "10px 16px",
            background: focusStudyArea ? "#DBEAFE" : "#FFFFFF",
            color: "#0F172A",
